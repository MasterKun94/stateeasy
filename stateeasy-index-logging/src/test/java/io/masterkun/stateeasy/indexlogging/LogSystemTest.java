...
public class LogSystemTest {
    ...
    public void test() throws Exception {
        ...
        io.masterkun.stateeasy.indexlogging.EventLogger<String> logger = system.get(config,
                new StringSerializer());

        var observer = new TestObserver();
        logger.read(0, 10, observer);
        long start = System.currentTimeMillis();
        io.masterkun.stateeasy.indexlogging.IdAndOffset nextIdAndOffset =
                observer.getFuture().join();
        long last = System.currentTimeMillis() - start;
        Assert.assertTrue("last: " + last, last > 180 && last < 220);
        Assert.assertEquals(0, nextIdAndOffset.id());

        CompletableFuture<io.masterkun.stateeasy.indexlogging.IdAndOffset> future = logger.write(
                "Hello");
        ...
        Assert.assertThrows(OverlappingFileLockException.class, () -> system2.get(config,
                new StringSerializer()));
    ...
    private static class TestObserver implements LogObserver<String> {
        ...
        private final CompletableFuture<io.masterkun.stateeasy.indexlogging.IdAndOffset> future =
        ...
        public void onComplete(long nextId, long nextOffset) {
            ...
            future.complete(new io.masterkun.stateeasy.indexlogging.IdAndOffset(nextId,
                    nextOffset));
        }
