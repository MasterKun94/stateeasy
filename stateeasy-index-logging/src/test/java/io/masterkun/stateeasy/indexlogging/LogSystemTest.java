...
public class LogSystemTest {
    ...
    public void test() throws Exception {
        // TODO
        io.masterkun.stateeasy.indexlogging.LogSystem system = new io.masterkun.stateeasy.indexlogging.LogSystem(2);
        ...
        io.masterkun.stateeasy.indexlogging.EventLogger<String> logger = system.get(config, new StringSerializer());

        var observer = new TestObserver();
        logger.read(0, 10, observer);
        long start = System.currentTimeMillis();
        io.masterkun.stateeasy.indexlogging.IdAndOffset nextIdAndOffset = observer.getFuture().join();
        long last = System.currentTimeMillis() - start;
        Assert.assertTrue("last: " + last, last > 180 && last < 220);
        Assert.assertEquals(0, nextIdAndOffset.id());

        CompletableFuture<io.masterkun.stateeasy.indexlogging.IdAndOffset> future = logger.write("Hello");
        Assert.assertFalse(future.isDone());
        start = System.currentTimeMillis();
        Assert.assertThrows(TimeoutException.class, () -> future.get(10, TimeUnit.MILLISECONDS));
        io.masterkun.stateeasy.indexlogging.IdAndOffset idAndOffset = future.join();
        ...
        io.masterkun.stateeasy.indexlogging.LogSystem system2 = new LogSystem(2);
    ...
    private static class TestObserver implements LogObserver<String> {
        ...
        private final CompletableFuture<io.masterkun.stateeasy.indexlogging.IdAndOffset> future = new CompletableFuture<>();
        ...
        public CompletableFuture<io.masterkun.stateeasy.indexlogging.IdAndOffset> getFuture() {
            return future;
        }
    }
