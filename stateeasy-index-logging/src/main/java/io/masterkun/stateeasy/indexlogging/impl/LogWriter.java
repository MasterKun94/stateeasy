 ...
 * {@link MappedByteBufferLogWriter} and provides methods for opening a data output, flushing the buffer,
 * adding listeners, and closing the writer.
 *
...
sealed interface LogWriter extends Closeable permits MappedByteBufferLogWriter {
     ...
     * @param autoFlushSize           the size in bytes at which the buffer should be automatically flushed
     * @param autoFlushInterval       the interval at which the buffer should be automatically flushed
     ...
     *
     * @param immediateFlush if true, the buffer will be flushed immediately after the DataOut is closed; otherwise, it will
     *                       be flushed according to the auto-flush settings
    ...
    /**
     * Forces any buffered data to be written to the underlying storage. This method ensures that all data
     * currently in the buffer is flushed and made persistent, regardless of the auto-flush settings.
     */
    void flush();
     ...
     * @param timeoutMills the timeout duration in milliseconds after which the listener's onTimeout method will be called
