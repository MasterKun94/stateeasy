...
/**
 * A sealed interface for managing and indexing log data. This interface provides methods for updating, retrieving,
 * and checking the state of a log index. It is designed to be implemented by classes that handle specific log indexing
 * strategies.
 *
 * <p>The {@code LogIndexer} interface is sealed, allowing only the specified implementations: {@code ChunkedLogIndexer}
 * and {@code MappedByteBufferLogIndexer}.
 */
public sealed interface LogIndexer permits ChunkedLogIndexer, MappedByteBufferLogIndexer {
    ...
    /**
     * Returns the offset of the log entry with the given identifier, or the offset of the closest preceding log entry if the exact identifier is not found.
     *
     * @param id the identifier of the log entry
     * @return the offset of the log entry with the given identifier or the closest preceding log entry
     */
