...
import io.stateeasy.concurrent.HasMetrics;
import io.stateeasy.indexlogging.impl.EventLoggerImpl;
...
/**
 * A logging system that manages and provides {@link EventLogger} instances based on provided
 * configurations. It ensures that each logger is uniquely identified by its configuration,
 * serializer, and optional reader executor. The system also supports metrics registration for
 * monitoring purposes.
 */
public final class LogSystem implements HasMetrics {
    ...
    public LogSystem(int threadNumPerDisk) {
        ...
        this.executorPool = new ExecutorPool(threadNumPerDisk, (e, r) -> {
            EventExecutorThread thread = new EventExecutorThread(e, r);
     ...
     * @throws IllegalArgumentException if a logger with different parameters already exists for the
     *                                  given configuration
     */
    ...
    /**
     * Retrieves or creates an {@link EventLogger} for the given configuration, serializer, and
     * optional reader executor.
     *
     * @param <T>            the type of the objects to be logged
     * @param config         the configuration for the logger
     * @param serializer     the serializer for converting objects to a byte stream
     ...
     * @throws IOException              if an I/O error occurs during the creation of the logger
     * @throws IllegalArgumentException if a logger with different parameters already exists for the
     *                                  given configuration or if the system is already shut down
     */
    public <T> EventLogger<T> get(LogConfig config,
        ...
        try {
            ...
            return (EventLogger<T>) cache.compute(config.name(), (key, holder) -> {
                if (holder == null) {
                    ...
                    throw new IllegalArgumentException("Logger already exist and parameters not " +
                            "match");
                }
            }).logger();
     ...
     * current LogSystem. This method calls the overloaded {@code register} method with executor
     * metrics disabled.
     *
     ...
     * timers, to a monitoring system. The metrics are registered with the specified prefix and
     * additional tags. It also adds two additional tags: "log_system_id" and the instance ID of the
     * current LogSystem. If executorMetricsEnabled is true, it also registers metrics for the
     * executor pool.
     *
     * @param metricPrefix           the prefix to be used for all metrics registered by this
     *                               component
     * @param registry               the MeterRegistry to which the metrics will be registered
     * @param executorMetricsEnabled a flag indicating whether to register metrics for the executor
     *                               pool
     * @param tags                   optional tags to be associated with the metrics
     */
    public void register(String metricPrefix, MeterRegistry registry,
                         boolean executorMetricsEnabled, String... tags) {
